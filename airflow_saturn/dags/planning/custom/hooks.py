...
class MovielensHook(BaseHook):
    ...
    Abstracts details of the Movielens (REST) API and provides several
    convenience methods for fetching data (e.g. ratings, users, movies)

    :param conn_id: ID of the connection to use to connect to the
        Movielens API. Connection is expected to include authentication details
    ...
    def get_conn(self):
        ...
        if self._session is None:
            ...
            if not config.host:
                raise ValueError(
                    f"No host specified in connection {self._conn_id}")
